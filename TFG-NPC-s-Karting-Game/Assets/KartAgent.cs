...
#region Rewards
        ...
        public float TowardsCheckpointReward = 0.05f;
        [Tooltip("Typically if the agent moves faster, we want to reward it for finishing the track quickly.")]
        public float SpeedReward = 0.2f;
        [Tooltip("Reward the agent when it keeps accelerating")]
        public float AccelerationReward = 0.01f;
        [Tooltip("What pently is given when the agent crashes with object")]
        public float CrashObjectPenalty = -0.85f;
        [Tooltip("What penaltly is given when the agent pass the wrong checkpoints")]
        public float PassWrongCheckPointPenalty = -0.5f;
...
#region Debugging
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            // Si va vuelta atras es decir va en los checkpoints que ya habia pasado entonces le resta
            if (triggered > 0 && index < m_CheckpointIndex || m_CheckpointIndex == 0 && index == Colliders.Length - 1)
            {
                AddReward(PassWrongCheckPointPenalty);
            }
            ...
            for (var i = 0; i < Sensors.Length; i++)
            {
                ...
                if (hit)
                {
                    if (hitInfo.distance < current.HitValidationDistance)
                    {
                        ...
                        if (LayerMask.LayerToName(layer) == "Track")
                        {
                            m_LastAccumulatedReward += HitPenalty;
                        }

                        if (LayerMask.LayerToName(layer) == "CrashObject")
                        {
                            m_LastAccumulatedReward += CrashObjectPenalty;
                        }
